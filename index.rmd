...
```{r wband, warning = FALSE, message = FALSE, results = 'asis', fig.height=9, fig.width=13}
...
### Trends {.tabset}
```{r tr, warning = F, message = F, fig.height=9, fig.width=13, results = 'asis'}
sonde_params <- c("DO_mgL", "pH", "Temp_C", "SpCond_uScm")

plotTrend(park = park, years = yr_start:yr_current, parameter = sonde_params, 
          smooth = T, span = 0.5, legend_position = "bottom")

cat(paste0("<p style='width:1280px;'>Trends in physical water quality parameters for ", sitename, " from ",
...
```{r}
drgt_cumppt_cap1 <- paste0("<p style = 'width:1280px;>Figure on the left shows weekly drought condition based on proportion of a park's predominant county by drought level using the <a href='https://droughtmonitor.unl.edu/'>U.S. Drought Monitor</a>. Figure on the right shows cumulative annual precipitation by month compared to the 20th century baseline. Total monthly precipitation data are derived from  <a href='https://www.ncei.noaa.gov/access/metadata/landing-page/bin/iso?id=gov.noaa.ncdc:C00332'>NOAA monthly NClimGrid gridded climate data</a> extracted for a given park's geographic centroid.</p>") 

anom_comp_cap1 <- paste0("<p style = 'width:1280px;'>Figure on the left compares current monthly climate data (solid) with historic baselines (dashed). Figure on the right represents monthly deviations from historic baselines, with positive (red) bars indicating above average deviations, and negative (blue) bars indicating below average deviations. Historic baselines are based on <a href='https://www.ncei.noaa.gov/products/land-based-station/us-climate-normals'>NOAA 20th century normals</a> for a given park's geographic centroid. Current monthly data are derived from <a href='https://www.ncei.noaa.gov/access/metadata/landing-page/bin/iso?id=gov.noaa.ncdc:C00332'>NOAA NClimGrid </a> gridded climate dataset extracted for a given park's geographic centroid.</p>") 

anom_cap5 <- paste0("<p style = 'width:1280px;'>Figure represents monthly deviations from historic baselines, with positive (red) bars, indicated above average deviations, and negative (blue) bars indicating below average deviations. Historic baselines are based on <a href='https://www.ncei.noaa.gov/products/land-based-station/us-climate-normals'>NOAA 20th century normals</a> for a given park's geographic centroid. 5-year monthly data are derived from <a href='https://www.ncei.noaa.gov/access/metadata/landing-page/bin/iso?id=gov.noaa.ncdc:C00332'>NOAA NClimGrid </a> gridded climate dataset extracted for a given park's geographic centroid.</p>")

cum_cap5 <- paste0("<p style = 'width:1280px;'>Figure shows cumulative annual precipitation by month for past 5 years compared to the 20th century baseline. Total monthly precipitation data are derived from  <a href='https://www.ncei.noaa.gov/access/metadata/landing-page/bin/iso?id=gov.noaa.ncdc:C00332'>NOAA monthly NClimGrid gridded climate data</a> extracted for a given park's geographic centroid.</p>")

drgt_cap5 <- paste0("<p style = 'width:1280px;'>Figure shows last 5 years of weekly drought condition based on proportion of a park's predominant county by drought level using the <a href='https://droughtmonitor.unl.edu/'>U.S. Drought Monitor</a>.</p>") 

temp_capT <- paste0("<p style = 'width:1280px;'>Figure shows loess smoothed (span 0.3) minimum, maximum, and average temperature trends by month derived from <a href='https://www.ncei.noaa.gov/access/metadata/landing-page/bin/iso?id=gov.noaa.ncdc:C00332'>NOAA NClimGrid </a> gridded climate dataset extracted for a given park's geographic centroid.</p>")

ppt_capT <- paste0("<p style = 'width:1280px;'>Figure shows loess smoothed (span 0.3) total precipitation trends by month derived from <a href='https://www.ncei.noaa.gov/access/metadata/landing-page/bin/iso?id=gov.noaa.ncdc:C00332'>NOAA NClimGrid </a> gridded climate dataset extracted for a given park's geographic centroid.</p>")

anom_capT <- paste0("<p style = 'width:1280px;'>Figure represents monthly deviations from historic baselines, with positive (red) bars, indicated above average deviations, and negative (blue) bars indicating below average deviations. Historic baselines are based on <a href='https://www.ncei.noaa.gov/products/land-based-station/us-climate-normals'>NOAA 20th century normals</a> for a given park's geographic centroid. Monthly data are derived from <a href='https://www.ncei.noaa.gov/access/metadata/landing-page/bin/iso?id=gov.noaa.ncdc:C00332'>NOAA NClimGrid </a> gridded climate dataset extracted for a given park's geographic centroid.</p>")

cum_capT <- paste0("<p style = 'width:1280px;'>Figure shows cumulative annual precipitation by month for past 5 years compared to the 20th century baseline. Total monthly precipitation data are derived from  <a href='https://www.ncei.noaa.gov/access/metadata/landing-page/bin/iso?id=gov.noaa.ncdc:C00332'>NOAA monthly NClimGrid gridded climate data</a> extracted for a given park's geographic centroid.</p>")

drgt_capT <- paste0("<p style = 'width:1280px;'>Figure shows last 5 years of weekly drought condition based on proportion of a park's predominant county by drought level using the <a href='https://droughtmonitor.unl.edu/'>U.S. Drought Monitor</a>.</p>") 
```
...
temp_comp <- plotClimComps(park = park, years= yr_current, palette = 'red', parameter = c('tmean'),
                           legend_position = 'bottom', plot_title = F)
...
```{r fig.height=7, fig.width=13, results = 'asis'}
drgt <- plotClimDrought(park = park, years = yr_current, legend_position = 'bottom')
pptcum <- plotClimCumPrecip(park = park, years = yr_current, legend_position = "bottom")
grid.arrange(drgt, pptcum, nrow = 1)
cat("\n\n")

cat(drgt_cumppt_cap1, "\n\n")
```
</details>
<br>
...
yr1 = yr_current - 4
plotClimAnom(park = park, years = yr1:yr_current, parameter = 'tmean', legend_position = "bottom", gridlines = "grid_x")
cat(anom_cap5, "\n\n")
```
</details>
<br>
...
```{r fig.height=8, fig.width=13, results = 'asis'}
plotClimAnom(park = park, years = yr1:yr_current, parameter = 'ppt', legend_position = "bottom", gridlines = "grid_x")
cat(anom_cap5, "\n\n")
```
</details>
<br>
...
```{r fig.height=8, fig.width=13, results = 'asis'}
plotClimCumPrecip(park = park, years = yr1:yr_current, legend_position = "bottom")
...
plotClimDrought(park = park, years = yr1:yr_current, gridlines = "grid_y", legend_position = "bottom")
cat(drgt_cap5, "\n\n")
```
</details>
</br>
...
plotClimTrend(park = park, years = 1900:yr_current, 
              ...
              palette = "Set2", legend_position = "bottom", layers = 'smooth')
cat(temp_capT, "\n\n")
...
plotClimAnom(park = park, years = 1900:yr_current, 
              parameter = c("tmean"), legend_position = "bottom")
...
```{r fig.height=8, fig.width=13, results = 'asis'}
plotClimAnom(park = park, years = 1900:yr_current, 
              parameter = c("ppt"), legend_position = "bottom")
