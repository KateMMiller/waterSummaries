...
<!-- Move to index after working -->
```{r}
param_list <- c("DO_mgL", "Temp_C", "pH", "SpCond_uScm", "ChlA_ugL")#, "Turbidity_FNU")
param_name <- c("Dissolved Oxygen (mgL)", "Temperature (C)", "pH", "Specific Conductance (uScm)",
                "Chlorophyll A (ugL)")#, "Turbidity (FNU)")
param_df <- data.frame(param = param_list, param_name = param_name)
```
...
cat("\n\n")
prof_splot <- plotLakeProfile(site = lake_list, years = yr_current, parameter = param)
print(prof_splot)
cat("\n\n")
