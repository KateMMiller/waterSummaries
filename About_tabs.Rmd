...
```{r chunk_control}
# Create controls that turn chunks on/off depending on the park code and category

parkcode <- params$park
category <- c("nutrients", "physical")

# Find state that park is in
state <- case_when(
  parkcode == "MIMA" | parkcode == "SAIR" ~ "MA",
  parkcode == "ROVA"| parkcode == "SARA" ~ "NY",
  parkcode == "ACAD" ~ "ME",
  parkcode == "MABI" ~ "VT",
  parkcode == "MORR" ~ "NJ",
  parkcode == "SAGA" ~ "NH",
...
# List of all states
state_list <- c("CT", "MA", "ME", "NH", "NJ", "NY", "VT")

# If state is in state_list, give it the value TRUE. All other states FALSE.
# This turns on the appropriate state water quality chunk via 'include ='
for (i in state_list) {
  assign(i, if_else(i == state, TRUE, FALSE))
}

# Same concept for category controls
category_list <- c("physical", "nutrients")

for (i in category_list){
  assign(i, if_else(i == category, TRUE, FALSE))
}
...
htmltools::withTags(body(
  ...
  h3("Specific Conductance"), p("Specific conductance is a temperature-corrected measure of the electrical conductivity of water and is directly related to ion concentration. The capacity of water to conduct an electrical current, i.e. its conductivity, is highly dependent on temperature and may change as much as 3% for each 1", degree, "C change. Thus, it is necessary to correct for temperature because a significant change in conductivity may simply be due to water temperature and not due to changes in ions in the water."),
...
```{r connecticut, results = 'asis', include = CT}
...
```{r maine, results = 'asis', include = ME}
...
```{r massachusetts, results = 'asis', include = MA}
...
```{r new_hampshire, results = 'asis', include = NH}
...
```{r new_jersey, results = 'asis', include = NJ}
...
```{r new_york, results = 'asis', include = NY}
...
```{r vermont, results = 'asis', include = VT}
